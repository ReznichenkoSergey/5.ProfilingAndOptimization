...
namespace MyCalculatorv1
{
	public class MainWindow : Window, IComponentConnector
	{
        ...
        /// Fix result method 
        /// </summary>
		private void resultFix()
        {
			var text = tb.Text;
			var values = text.Split(new string[] { "+", "-", "*", "/" }, StringSplitOptions.RemoveEmptyEntries);
			
			var result = "ERROR";
			if (values.Length == 2 && double.TryParse(values[0], out double num1) && double.TryParse(values[1], out double num2))
			{
				var sign = text.Substring(values[0].Length, 1);
				result = sign switch
				{
					"+" => $"{text} = {num1 + num2}",
					"-" => $"{text} = {num1 - num2}",
